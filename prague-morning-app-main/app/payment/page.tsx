...
const Payment = () => {
  ...
  const [servicePlan,setServicePlan]=useState<number | undefined>(150)

  return (
    <section className={styles["payment-page"]}>
      <div className={styles["payment-page__wrapper"]}>
        <PaymentPlanCheckbox monthly={monthly} yearly={yearly} setMonthly={setMonthly} setYearly={setYearly}/>
        <ServicePlans servicePlan={servicePlan} setServicePlan={setServicePlan}/>
        {!!servicePlan && <PaymentContainer servicePlan={servicePlan}/>}
      </div>
    </section>
  );
};
