...
async function getData(params: any) {
  const res = await fetch(
    `https://prague-morning-backend.vercel.app/api/jobs?${params}`,
    { cache: 'no-store' }
  );
  if (!res) {
...
async function getOptions() {
  const res = await fetch(
    `https://prague-morning-backend.vercel.app/api/job-options`,
    { cache: 'no-store' }
  );
...
const Jobs = async ({ searchParams }: JobsPagePropsTypes) => {
  ...
  const defaultSalary = uniqueArray(salary)?.find(
    (item) => item.label == searchParams?.salary
  );
